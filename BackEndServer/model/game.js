...
var gameDB = {
  ...
  insertGame: function (title, game_description, year, game_image, callback) {
    ...
    dbConn.connect(function (err) {
      if (err) {
        ...
        const insertGameSql = `INSERT INTO game (title, game_description, year, game_image) VALUES (?, ?, ?, ?);`;
        const queryValues = [title, game_description, year, game_image.buffer];

        dbConn.query(insertGameSql, queryValues, function (err, results) {
          if (err) {
            dbConn.end();
            return callback(err, null);
          } else {
    ...
    dbConn.connect(function (err) {
      if (err) {
        ...
        for (var i = 0; i < platformArr.length; i++) {
          ...
          var insertGamePlatSql =
            "INSERT INTO game_platform (fk_game, fk_platform, price) VALUES (?, ?, ?)";
          dbConn.query(
            insertGamePlatSql,
            [gameID, newplatformid, newprice],
    ...
    dbConn.connect(function (err) {
      if (err) {
        ...
        for (var i = 0; i < categoryArr.length; i++) {
          ...
          var insertGameCatSql =
            "INSERT INTO game_category (fk_game, fk_category) VALUES (?, ?)";
          dbConn.query(
            insertGameCatSql,
            [gameID, newcategoryid],
            function (err, results) {
              if (err) {
                dbConn.end();
                return callback(err, null);
  ...
  get_Plat_ID: function (gameID, callback) {
    ...
    dbConn.connect(function (err) {
      if (err) {
        ...
        var getPlatIDSql =
          "SELECT game_platformID FROM game_platform WHERE fk_game=?";
  ...
  get_Cat_ID: function (gameID, callback) {
    ...
    dbConn.connect(function (err) {
      if (err) {
        ...
        var getCatIDSql =
          "SELECT game_categoryID FROM game_category WHERE fk_game=?";
  ...
  updatePlatform: function (gameID, platformid, price, Plat_IDs, callback) {
    ...
    dbConn.connect(function (err) {
      ...
      var updateSql =
        "UPDATE game_platform SET fk_platform=?, price=? WHERE game_platformID=?";
      ...
      if (platformArr.length === Plat_IDs.length) {
        ...
        for (var i = 0; i < Plat_IDs.length; i++) {
          dbConn.query(
            updateSql,
            [platformArr[i], priceArr[i], Plat_IDs[i]],
            function (err, results) {
              if (err) {
                dbConn.end();
                return callback(err, results);
        ...
        for (var i = 0; i < platformArr.length; i++) {
          if (i < Plat_IDs.length) {
            ...
            dbConn.query(
              postSql,
              [gameID, platformArr[i], priceArr[i]],
              function (err, results) {
                if (err) {
                  dbConn.end();
                  return callback(err, results);
  ...
  updateCategory: function (gameID, categoryid, Cat_IDs, callback) {
    ...
    dbConn.connect(function (err) {
      ...
      var updateSql =
        "UPDATE game_category SET fk_category=? WHERE game_categoryID=?";
