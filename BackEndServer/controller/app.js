...
app.post("/users/logout", function (req, res) {
  ...
  logger.info("SUCCESSFUL_LOGOUT", { userId: req.body.userid, ip: req.ip });
      ...
      if (err) {
        ...
        if (err.code === "ER_DUP_ENTRY") {
          ...
          if (err.sqlMessage.includes("platform_name")) {
            ...
            res.type("json");
            res.send(
              `{"Message":"The platform name provided already exists."}`,
            );
          }
        } else {
          console.log(err);
          ...
          if (err) {
            ...
            gameDB.insertGame_Category(gameID, categoryid, function (err) {
              if (err) {
                ...
                res.type("json");
                res.send(
                  `{"Message":"Internal Server Error with game_category"}`,
                );
              } else {
                res.status(201);
                res.type("json");
